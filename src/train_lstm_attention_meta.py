"""
train_lstm_attention_meta.py
-----------------------------
Single-split training loop for ``lstm.LSTMAttentionMetaEmbModel`` — a direct
adaptation of ``train_lstm_attention.py`` (which trains ``lstm.LSTMAttentionRec``)
so that pretrained item-metadata embeddings (``src/build_item_embeddings.py``)
are actually fused into the token embedding via ``LSTMAttentionMetaEmbModel``.

Differences from ``train_lstm_attention.py``:
  * Model class swapped to ``LSTMAttentionMetaEmbModel``, which takes the
    ``model`` config section directly (plus a tokenizer) instead of individual
    kwargs, since it needs ``cfg["use_meta_embeddings"]`` /
    ``cfg["item_meta_embedding"]`` / ``cfg["item_meta_id_map"]`` to build the
    frozen metadata embedding table.
  * Fixed the ``--source`` validation: the original asserted
    ``args.source not in [...]``, which is inverted — it raised
    ``AssertionError`` for every *valid* choice (including the default
    ``dressipi``), so the script could never actually run. Now asserts
    ``args.source in [...]``, matching ``train_cv_lstm_attention.py``.
  * Fixed the closing log line referencing the module-level ``args`` (which
    only exists when the script is run as ``__main__``) instead of the
    ``eval_k`` parameter already in scope — calling ``train_model()`` directly
    (e.g. from a notebook or another script) would otherwise raise
    ``NameError``.

Usage:
    python src/train_lstm_attention_meta.py --source dressipi
    python src/train_lstm_attention_meta.py --source dressipi --eval-k 20 -v
"""
...
from lstm import LSTMAttentionMetaEmbModel
...
def train_model(cfg: dict, eval_k: int, verbose: bool):
    ...
    if cfg_hyperparam["use_adaptive_softmax"]:
        ...
        model = LSTMAttentionMetaEmbModel(cfg_model, tokenizer=tokenizer).to(device)
    ...
    if os.path.exists(checkpoint_path):
        # weights_only=False: this is a full training checkpoint (optimizer/scheduler
        # state, epoch, numpy-typed loss/ndcg), not just a tensor state_dict, and it's
        # our own trusted file — torch>=2.6 defaults weights_only=True, which can't
        # unpickle non-tensor objects like numpy scalars.
    ...
    print(f"done. best val NDCG@{eval_k}={best_ndcg:.4f}  weights → {checkpoint_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train a LARA model (LSTMAttentionMetaEmbModel)')
