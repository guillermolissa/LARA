...
def reset_weights(m):
  '''
    ...
    Skips ``meta_emb``: it holds pretrained, frozen item-metadata embeddings
    loaded from disk at model construction time — resetting it here would
    overwrite them with random noise that (being frozen) never trains back.
  '''
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train a LARA model (LSTMAttentionMetaEmbModel) using CV on the dressipi dataset')
